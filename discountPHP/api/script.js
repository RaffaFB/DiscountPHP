...
function validador(event){
    ...
    if (errorMessage !== ""){
        ...
        // Aqui, fazemos a chamada AJAX para o PHP
        sendDataToServer(principalValue, finalValue, time, monthlyRate, checkbox, entryValue, monthsToBack, valueToBack);

        event.preventDefault();
    }
}

function sendDataToServer(principalValue, finalValue, time, monthlyRate, checkbox, entryValue, monthsToBack, valueToBack) {
    // Crie um objeto FormData para enviar os dados ao servidor
    const formData = new FormData();
    formData.append('monthlyRate', monthlyRate);
    formData.append('principalValue', principalValue);
    ...
    // Configure o retorno de chamada para quando a solicitação estiver concluída
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            if (response.success) {
                // Os dados da tabelaPrice estarão em response.tabelaPrice
                const tableData = response.tabelaPrice;

                // Continue com a lógica para popular a tabela e mostrar os dados
                const table = tabelaPrice(principalValue, time, monthlyRate, entryValue, 0);
                popularTabela(table, finalValue, principalValue, entryValue);
                const coefficient = calculatefinancingCoefficient(monthlyRate, time);
                mostrarDados(monthlyRate, principalValue, finalValue, time, 0, checkbox, coefficient, valueToBack, monthsToBack, 0, entryValue);
            } else {
                // Lide com erro do servidor, se necessário
